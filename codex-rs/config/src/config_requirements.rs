...
pub struct PluginsRequirementsToml {
    ...
    #[serde(flatten)]
    pub entries: BTreeMap<String, PluginRequirementsToml>,
}

impl PluginsRequirementsToml {
    pub fn is_empty(&self) -> bool {
        self.allow_sharing.is_none() && self.entries.values().all(PluginRequirementsToml::is_empty)
    }
}
...
"#,
    ...
    fn plugin_mcp_requirements(plugin_name: &str) -> PluginsRequirementsToml {
        PluginsRequirementsToml {
            ...
            entries: BTreeMap::from([(
                ...
                PluginRequirementsToml { mcp_servers: None },
            )]),
        }
    }
    ...
    fn merge_plugin_requirements_descending_prefers_false_from_lower_precedence() {
        let mut merged = plugin_requirements(Some(true));
        let lower = plugin_requirements(Some(false));

        merge_plugin_requirements_descending(&mut merged, lower);

        assert_eq!(merged, plugin_requirements(Some(false)));
    }
    ...
    fn merge_unset_fields_plugins_empty_higher_source_does_not_block_lower_sharing_disable() {
        ...
        target.merge_unset_fields(
            RequirementSource::CloudRequirements,
            ConfigRequirementsToml {
                plugins: Some(plugin_requirements(None)),
                ..Default::default()
            },
        );
        target.merge_unset_fields(
            RequirementSource::LegacyManagedConfigTomlFromMdm,
            ConfigRequirementsToml {
                plugins: Some(plugin_requirements(Some(false))),
                ..Default::default()
            },
        );
        ...
        assert_eq!(
            ...
            Some(plugin_requirements(Some(false))),
        );
    }
