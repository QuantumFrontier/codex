...
pub(crate) struct ExtensionToolHandler {
    executor: Arc<ExtensionToolExecutor>,
}

impl ExtensionToolHandler {
    pub(crate) fn new(executor: Arc<ExtensionToolExecutor>) -> Self {
        Self { executor }
    }
...
impl ToolExecutor<ToolInvocation> for ExtensionToolHandler {
    ...
    async fn is_mutating(&self, invocation: &ToolInvocation) -> bool {
        self.executor
            .is_mutating(&to_extension_call(invocation))
            .await
    }
...
mod tests {
    ...
    impl codex_extension_api::ToolExecutor<codex_tools::ToolCall> for StubExtensionExecutor {
        type Output = codex_extension_api::ExtensionToolOutput;
        ...
        fn spec(&self) -> Option<codex_tools::ToolSpec> {
            Some(codex_tools::ToolSpec::Function(codex_tools::ResponsesApiTool {
        ...
        async fn is_mutating(&self, _call: &codex_tools::ToolCall) -> bool {
            true
        }
